class JFlow {
            ...
            if (css) {
                ...
                for (let i = 0; i < imgTagList.length; i++) {
                    if (!imgTagList[i]) {
                        if (i == 0) {
                            let div = document.createElement("div");
                            div.setAttribute("class", i == 0 ? `nm_img ${styleName} ${op.keyName}` : `hl_img ${styleName} ${op.keyName}`);
                            div.style.position = "absolute";
                            div.style.left = "0px";
                            div.style.top = "0px";
                    ...
                    img.setAttribute("class", i == 0 ? `nm_img ${op.keyName} ${styleName}` : `hl_img ${op.keyName} ${styleName}`);
    ...
    async decodeBoard_List(this: JMain, data: BoardListType, keyName: string) {
        ...
        for (let i = 0; i < num; i++) {
            ...
            for (let j = 0; j < imgTagList.length; j++) {
                ...
                edata.e.setAttribute("class", j == 0 ? `nm_img ${styleName} ${keyName}` : `hl_img ${styleName} ${keyName}`);
    ...
    async decodeBoard_ImgStyle(this: JMain, op: {
        ...
        for (let i = 0; i < imgTagList.length; i++) {
            ...
            edata.e.setAttribute("class", i == 0 ? `nm_img ${styleName} ${op.keyName}` : `hl_img ${styleName} ${op.keyName}`);
